...
class Diagnoses:
    """
    The Diagnoses class creates an alias called diagnoses_cd that transposes the 12 dgns_cd columns allowing :meth:`Enrollment.Enrollment.having` to filter
    by various diagnoses types. It also plays a role in the backend method for decorating the chronic conidition column the user specifies.
    ...
    def where(service_category: ServiceCategory, diagnoses: list):
        ...
        a service category from the :class:`ServiceCategory` class and the list of diagnosis codes they have specified. 

        Args:
            service_category: `attribute`: Specify an attribute from the :class:`ServiceCategory` such as ServiceCategory.inpaitent
            diagnoses: `list`: User defined list of diagnosis codes
