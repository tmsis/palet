...
# MAGIC %md
# MAGIC ##### Count enrollees by month that have ICD-10 codes at least once in inpatient or twice in other services.
...
# MAGIC %md
# MAGIC ##### Creating multiple indicators
...
display(api_mark.fetch())

# COMMAND ----------

api_re = Enrollment([6280]).byMonth()            \
  .byAgeRange()                                  \
  .byEnrollmentType()                            \
  .mark(                                         \
    Diagnoses.within([                           \
...
display(df[['year','month','enrollment_type_label','ageGroup','Diabetes','AFib','mdcd_enrollment']])
